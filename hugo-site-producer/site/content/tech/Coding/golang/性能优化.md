...
for _, task := range tasks {
    go process(task)  // 可能创建太多 goroutine
}
...
```go
// 根据场景选择协议
- REST/HTTP/1.1 → 简单，兼容性好
- HTTP/2 → 多路复用，头部压缩
- gRPC → 高性能，protobuf 编码
- WebSocket → 实时双向通信
```
