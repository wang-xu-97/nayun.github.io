...
plt.rcParams['font.sans-serif'] = ['AR PL UMing CN']  # 指定默认字体为黑体
...
def parse_args():
    parser = argparse.ArgumentParser(description='技能效果对比分析')
    parser.add_argument('-e', '--enable', default="jwqds", choices=skills, nargs='+', 
                       help='启用技能，默认启用巨武器大师')
    parser.add_argument('-d', '--dice', type=lambda v:dice(*v.split('d')), default="1d12", 
                       help='武器伤害骰')
    parser.add_argument('-m', '--mode', choices=['single', 'compare'], default='compare',
                       help='分析模式: single-单函数分析, compare-多函数对比')
    return parser.parse_args()

def single_function_analysis(func_dict, coord_info, a_vals, b_vals):
    """单函数分析模式"""
    if not func_dict:
        print("错误: 没有找到可分析的函数")
        return
    
    func_name, func = list(func_dict.items())[0]
...
def multi_function_compare(func_dict, coord_info, a_vals, b_vals):
    """多函数对比模式"""
    if len(func_dict) < 2:
...
def main():
    ...
    # 获取函数字典和坐标信息
    func_dict, coord_info = factory(args)
    print("可用的函数:", list(func_dict.keys()))
    ...
    # 根据模式选择分析方式
    if args.mode == 'single':
        single_function_analysis(func_dict, coord_info, a_vals, b_vals)
    else:  # compare mode
